...
public: // triggers
  ...
  uint32_t 
  convertNonceToUint32(const ndn::Interest::Nonce& nonce);
...
private:
  ...
  std::unordered_map<uint32_t, std::chrono::steady_clock::time_point> seenNonces; // Nonceを管理するマップ
  static const std::chrono::milliseconds NONCE_RETENTION_TIME; // Nonceの保持時間
