...
private:
  ...
  //変更
  int rankContent(const fib::Entry& pitEntry,double threshold10, double threshold30, double threshold60);
  std::tuple<double, double, double> calculateFrequencyThreshold();
  bool shouldCacheData(int rank, double threshold);
  double calculateCentrality(uint32_t currentNodeId);
  double FixedcalculateCentrality(uint32_t currentNodeId);
  uint32_t GetHopCount(ns3::Ptr<ns3::Node> sourceNode, ns3::Ptr<ns3::Node> destinationNode);
  bool IsWithinRange(ns3::Ptr<ns3::Node> node1, ns3::Ptr<ns3::Node> node2);
