...
Forwarder::onIncomingInterest(const Interest& interest, const FaceEndpoint& ingress)
{
  ...
  // receive Interest //本来は最初に書く
  NFD_LOG_DEBUG("onIncomingInterest in=" << ingress << " interest=" << interest.getName()); 


  //Tagging the incoming face ID to the interest
  ...
  //PITに送信元のノードIDを設定
  int sendedID = interest.getNodeId();
  pitEntry->setSendedNodeId(sendedID);

  NFD_LOG_DEBUG("sended ID is  " << sendedID ); 

  // PITエントリの情報を出力
  
  int sub_node = ns3::Simulator::GetContext();
  //std::cout << "PIT Entry Updated:" << std::endl;
  //std::cout << "node is" << sub_node << std::endl;
  //std::cout << "  Name: " << pitEntry->getName() << std::endl;
  //std::cout << "  Access Frequency: " << std::fixed << std::setprecision(4) << pitEntry->getAccessFrequency() << std::endl;
  //std::cout << "  Hops: " << interest.getHops() << std::endl;
  ...
  if (ingress.face.getLinkType() == ndn::nfd::LINK_TYPE_POINT_TO_POINT) {
    ...
    //hasDuplicateNonceInPit = hasDuplicateNonceInPit && !(dnw & fw::DUPLICATE_NONCE_IN_SAME); //ループする際におかしくなる変更,有線p2pの際にループ判断しないようにする
  }
  if (hasDuplicateNonceInPit) {
    // goto Interest loop pipeline
    //this->onInterestLoop(interest, ingress); //ここで send-Nack-duplicate を送信する
    NFD_LOG_DEBUG("Nonce is duplicate loop "); 
    m_strategyChoice.findEffectiveStrategy(*pitEntry).afterReceiveLoopedInterest(ingress, interest, *pitEntry);
    return;
  }

  //アクセス頻度カウンターの更新
  uint32_t hops = interest.getHops();
  //std::cout << " Before Access Frequency: " << std::fixed << std::setprecision(4) << pitEntry->getAccessFrequency() << std::endl;
  pitEntry->updateAccessFrequency(hops);
                             ...
                             const shared_ptr<pit::Entry>& pitEntry, const Data& data)
{
  ...
  ///////////////////////////////////////////////////////////////////////
  //コンテンツがヒットしたときに、FIBアクセス頻度を増加

  // FIBエントリを取得
  const ndn::Name& dataName = data.getName();

  // FIBエントリを取得し、const_castでconstを解除
  auto& fibEntryConst = m_fib.findLongestPrefixMatch(dataName);
  auto* fibEntry = const_cast<nfd::fib::Entry*>(&fibEntryConst);

  fibEntry->updateFIBAccessFrequency(2);
...
Forwarder::onOutgoingInterest(const Interest& interest, Face& egress,
                              const shared_ptr<pit::Entry>& pitEntry)
{
  //送信元のノードIDをInterestパケットにセットする
  int node = ns3::Simulator::GetContext();
  const_cast<Interest&>(interest).setNodeId(node);
  ...
  // インクリメントされたホップ数を持つ新しいInterestを作成
  //int node = ns3::Simulator::GetContext();
  //std::cout << "Node " << node << " before received Interest with Hops value " << interest.getHops() << std::endl; 
  const_cast<Interest&>(interest).setHops(interest.getHops() + 1);
  //std::cout << "Node " << node << " after received Interest with Hops value " << interest.getHops() << std::endl; 
...
Forwarder::onIncomingData(const Data& data, const FaceEndpoint& ingress)
{
  ...
  uint32_t m_nodeId = ns3::Simulator::GetContext();
  if(m_nodeId > 500){  //無効なパケット（ネットワーク管理パケットなど）は破棄
    this->onDataUnsolicited(data, ingress);
    return;
  }
  else if(data.getName().size() > 500){   //同様に無効なパケット（ネットワーク管理パケットなどは破棄）
    this->onDataUnsolicited(data, ingress);
    return;
  }
  else {
    //アクセス頻度の閾値を計算
    double threshold10, threshold30, threshold60;
    std::tie(threshold10, threshold30, threshold60) = calculateFrequencyThreshold();  //4つのランクに分ける際の閾値を求める
  
    for (const auto& pitEntry : pitMatches) {
        // PITエントリの処理：受け取ったデータとPITエントリ内のデータがマッチしたときだけrankを取得する
        //pitEntry->getName()とdata.getName()は絶対に同じっぽい？
        //std::cout << " node " << m_nodeId << std::endl;
        //std::cout << " pitEntry->getName() " << pitEntry->getName() << std::endl;
        //std::cout << " data.getName() " << data.getName() << std::endl;
        //NFD_LOG_DEBUG("PIT comingnode is " << pitEntry->getNodeId());
        
        if(pitEntry->getName()==data.getName()){
          //PITとマッチした場合のみ、出力させる（転送する場合のみ）
          NFD_LOG_DEBUG("onIncomingData in=" << ingress << " data=" << data.getName());

          NFD_LOG_DEBUG("Content Name is " << data.getName());
          int rank = rankContent(*fibEntry, threshold10, threshold30, threshold60); //dataのコンテンツ人気度を取得
          //std::cout << " node " <<  m_nodeId << ": rank"  << rank << std::endl;
          double centrality = calculateCentrality(m_nodeId);         //該当ノードの近接中心性を求める(既存手法)         
          //double centrality = FixedcalculateCentrality(m_nodeId);    //該当ノードの近接中心性を求める(提案手法)

          // アクセス頻度の更新
          if (fibEntry->getPrefix().size() > 0) {
              fibEntry->updateFIBAccessFrequency(pitEntry->getAccessFrequency());  //pitEntry->getAccessFrequency()　:　アクセス頻度
          }

          bool shouldCache = shouldCacheData(rank, centrality); //（コンテンツ人気度、中心性）からキャッシュすべきか判断
          if (m_cs.size() < 11){  //キャッシュに空きがあった場合は、shouldCacheの値にかかわらずキャッシュする
            m_cs.insert(data);
          }
          else if (shouldCache) {
            m_cs.insert(data);
  ...
  // Check for satisfied PIT entries and forward data if Face is 257  //変更
  for (const auto& downstream : satisfiedDownstreams) {
    //std::cout << "down is  "<<  downstream.first->getId()  << std::endl;
    if (downstream.first->getId() == 257) {
      // 他のノードにデータを転送
      this->onOutgoingData(data, *downstream.first);
    }
    else if(downstream.first->getId() == 258){
      this->onOutgoingData(data, *downstream.first);
    }
  }
  ...
  for (const auto& downstream : satisfiedDownstreams) {
    //std::cout << "endpoint ID is: " << ingress.endpoint  << std::endl;
    ...
    //NFD_LOG_DEBUG("down steream is " << downstream.first->getId());

    //自分のノードからのリクエストはデータを転送しない
    bool testww = true;
    testww = downstream.first->getId() != 258;
    //NFD_LOG_DEBUG(testww);  
    std::cout << "endpoint Face 258 is: " << downstream.first->getId()  << std::endl;
    if(!testww){
      continue;
    }
    std::cout << "endpoint Face is: " << downstream.first->getId()  << std::endl;
    this->onOutgoingData(data, *downstream.first);
  }
}
...
Forwarder::calculateFrequencyThreshold()
{
  std::vector<double> frequencies;
  for (const auto& entry : m_fib) {
    frequencies.push_back(entry.getFIBAccessFrequency());
  }
  ...
  size_t thresholdIndex10 = frequencies.size() * 0.1; // 上位10%
  size_t thresholdIndex30 = frequencies.size() * 0.3; // 上位30%
  size_t thresholdIndex60 = frequencies.size() * 0.6; // 上位60%

  return std::make_tuple(frequencies[thresholdIndex10], frequencies[thresholdIndex30], frequencies[thresholdIndex60]);
}
////////////////////////////////////////////////
...
// ランク分け関数
int 
Forwarder::rankContent(const fib::Entry& fibEntry, double threshold10, double threshold30, double threshold60) {
  // PITからエントリを取得
    double frequency = fibEntry.getFIBAccessFrequency();
    NFD_LOG_DEBUG("access frequecy is =" << frequency);
    if (frequency >= threshold10) {
        return 0; // ランク1
    } else if (frequency >= threshold30) {
        return 1; // ランク2
    } else if (frequency >= threshold60) {
        return 2; // ランク3
    } else {
        return 3; // ランク4
    }
...
////////////////////////////////////////////////////////////////////////////////////////
//対象ノードが通信可能範囲に存在するか判断する関数
bool 
Forwarder::IsWithinRange(ns3::Ptr<ns3::Node> node1, ns3::Ptr<ns3::Node> node2)
{
    ...
    if (!mobility1 || !mobility2) return false;

    double distance = mobility1->GetDistanceFrom(mobility2);
    double range = 130.0;  // 通信範囲を60メートルと仮定（通信可能範囲はここを変更）
...
///////////////////////////////////////////////////////////////////////////////
//ホップ数を取得する（元ノード、宛先ノード　→　宛先ノードまでのホップ数）
uint32_t
Forwarder::GetHopCount(ns3::Ptr<ns3::Node> sourceNode, ns3::Ptr<ns3::Node> destinationNode){
    std::queue<std::pair<ns3::Ptr<ns3::Node>, uint32_t>> nodeQueue;
    std::set<uint32_t> visited;
    
    nodeQueue.push({sourceNode, 0});
    visited.insert(sourceNode->GetId());

    while (!nodeQueue.empty()) {
        auto [currentNode, hops] = nodeQueue.front();
        nodeQueue.pop();
        ...
        for (uint32_t i = 0; i < currentNode->GetNDevices(); ++i) {
            ns3::Ptr<ns3::NetDevice> device = currentNode->GetDevice(i);
            ns3::Ptr<ns3::Channel> channel = device->GetChannel();
            if (channel == nullptr) continue;

            for (uint32_t j = 0; j < channel->GetNDevices(); ++j) {
                ns3::Ptr<ns3::NetDevice> otherDevice = channel->GetDevice(j);
                ns3::Ptr<ns3::Node> neighborNode = otherDevice->GetNode();
                
                if (neighborNode == currentNode) continue;  // 自分自身はスキップ

                // 通信範囲内かどうかをチェック
                if (IsWithinRange(currentNode, neighborNode)) {
                    if (visited.find(neighborNode->GetId()) == visited.end()) {
                        nodeQueue.push({neighborNode, hops + 1});
                        visited.insert(neighborNode->GetId());
                        //std::cout << "source ID is "<< sourceNode->GetId() <<  " nodeQueue push  : " << neighborNode->GetId() << std::endl;
                    }
                }
            }
        }
    ...
    return std::numeric_limits<uint32_t>::max(); // 到達不可能な場合
}
/////////////////////////////////////////////////////////////

/////////////////////////////////////////////////////////////
//近接中心性を計算する（元ノード　→　元ノードの近接中心性）
double
Forwarder::calculateCentrality(uint32_t currentNodeId)
{
    uint32_t numNodes = ns3::NodeList::GetNNodes();
    if (currentNodeId >= numNodes) {
        NS_LOG_ERROR("Invalid node index: " << currentNodeId << ". Total nodes: " << numNodes);
        return -1.0;
    }

    ns3::Ptr<ns3::Node> currentNode = ns3::NodeList::GetNode(currentNodeId);
    uint32_t totalHops = 0;
    uint32_t reachableNodes = 0;

    for (uint32_t i = 0; i < numNodes; ++i) {
        if (i == currentNodeId) continue;
        ns3::Ptr<ns3::Node> otherNode = ns3::NodeList::GetNode(i);
        uint32_t hops = GetHopCount(currentNode, otherNode);
        if (hops < std::numeric_limits<uint32_t>::max()) {
          //std::cout << "hops is  : " << hops << std::endl;
            totalHops += hops;
            reachableNodes++;
        }
    }
    ...
    //std::cout << "current node ID "<< currentNodeId <<  " : totalhops "   << totalHops << " : totalnode  : "  << reachableNodes <<  std::endl;
    //std::cout << "totalhops is  : " << totalHops << std::endl;
    //std::cout << "totalnode is  : " << reachableNodes << std::endl;
    //double closeness = ((double)(reachableNodes - 1) * (reachableNodes - 1)) / ((numNodes - 1) * totalHops);
    double closeness = (double)(reachableNodes - 1) / totalHops;
    //std::cout << "closeness is  : " << closeness << std::endl;                   
    return closeness;
}
/////////////////////////////////////////////////////////////////////

// ノードIDからグループを取得する関数
uint32_t GetGroupId(uint32_t nodeId) {
    return nodeId / 10; // 12個ごとにグループ分け
}

/////////////////////////////////////////////////////////////////////
//　同じグループ内での近接中心性を取得する関数
double Forwarder::FixedcalculateCentrality(uint32_t currentNodeId) {
    uint32_t numNodes = ns3::NodeList::GetNNodes();
    if (currentNodeId >= numNodes) {
        NS_LOG_ERROR("Invalid node index: " << currentNodeId << ". Total nodes: " << numNodes);
        return -1.0;
    ...
    uint32_t totalHops = 0;
    uint32_t reachableNodes = 0;
    uint32_t currentGroupId = GetGroupId(currentNodeId);

    for (uint32_t i = 0; i < numNodes; ++i) {
        if (i == currentNodeId) continue;
        uint32_t otherGroupId = GetGroupId(i);

        // 異なるグループの場合はスキップ
        //std::cout << "currentGroup  : " << currentGroupId << std::endl;
        //std::cout << "otherGroup  : " << otherGroupId << std::endl;
        if (currentGroupId != otherGroupId) continue;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
//コンテンツの人気度rankとノードの中心性の値からキャッシュすべきか判断する
bool 
Forwarder::shouldCacheData(int rank, double centrality){
  //std::cout << "rank is  : " << rank << std::endl;
  //std::cout << "centrality is  : " << centrality << std::endl;
  if (rank == 3){
    return true;
  }
  else if (rank == 2){
    //return centrality > 0.7; //提案手法
    //return centrality > 0.3;   //既存手法
    return centrality > 0.8;   //テスト用
  }
  else if(rank == 1){
    //return centrality < 0.7; //提案手法
    //return centrality < 0.3;   //既存手法
