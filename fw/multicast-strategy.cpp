...
// Nonceの保存期間を2秒に設定
const std::chrono::milliseconds MulticastStrategy::NONCE_RETENTION_TIME(2000); // 修正
...
uint32_t 
MulticastStrategy::convertNonceToUint32(const ndn::Interest::Nonce& nonce) {
    // Nonceのバイトサイズを確認し、4バイト未満の場合はエラー処理
    if (nonce.size() < 4) {
        return 0; // 例外処理またはデフォルト値を返す
    }

    uint32_t value = 0;
    // Nonceのデータを取得し、最初の4バイトをuint32_tにコピー
    const uint8_t* nonceBytes = nonce.data(); // Nonceのデータを取得
    memcpy(&value, nonceBytes, sizeof(value)); // 最初の4バイトをコピー
                                        ...
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  uint32_t nonce = convertNonceToUint32(interest.getNonce());

  // Remove expired Nonces from the map
  for (auto it = seenNonces.begin(); it != seenNonces.end(); ) {
    if (now - it->second > NONCE_RETENTION_TIME) {
      it = seenNonces.erase(it); // Remove expired entries
    } else {
      ++it;
    }
  }

  // Check if this nonce has been seen recently
  if (seenNonces.find(nonce) != seenNonces.end()) {
    NFD_LOG_DEBUG("Suppressing Interest with seen Nonce: " << nonce);
    return; // If the nonce was already seen, suppress the Interest forwarding
  }

  // Add the current nonce to the map with a timestamp
