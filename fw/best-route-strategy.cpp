                                        ...
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  //NFD_LOG_DEBUG("Nonce number is  " << interest.getNonce());
  //std::cout << "Nonce number is  " << interest.getNonce() << std::endl;

  const fib::Entry& fibEntry = this->lookupFib(*pitEntry);
  const fib::NextHopList& nexthops = fibEntry.getNextHops();
  
  if (nexthops.empty()) {
  std::cout << "No NextHops available for this interest." << std::endl;
  }

  auto it = nexthops.end();  //必要

  if (suppression == RetxSuppressionResult::NEW) {
    // forward to nexthop with lowest cost except downstream
    // 新しいPITエントリがある場合、どのFaceにも転送可能
    it = std::find_if(nexthops.begin(), nexthops.end(), [&] (const auto& nexthop) {
      return true; // 同じFaceでも転送を許可
      //return isNextHopEligible(ingress.face, interest, nexthop, pitEntry); //ここ
    });
  ...
  // find an unused upstream with lowest cost except downstream//ここをコメントアウト（変更する場合は）
  ...
  //it = nexthops.begin();  // 先頭のネクストホップに転送:変更
